  ...
  return (
    selectedEvent ? (
      <div className="max-h-[70vh] overflow-y-auto">
        <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-2">{selectedEvent.title}</h2>
        <p className="text-gray-600 dark:text-gray-300 mb-2">{selectedEvent.date}</p>
        <p className="text-gray-700 dark:text-gray-200 mb-4">{selectedEvent.description}</p>
        <p className="text-gray-600 dark:text-gray-300 mb-4">Место: {selectedEvent.location}</p>
        ...
        <div className="mt-4">
          <h3 className="font-semibold text-gray-800 dark:text-white mb-2">Источники:</h3>
        ...
        <div className="mt-4">
          <h3 className="font-semibold text-gray-800 dark:text-white mb-2">Похожие события:</h3>
          <ul className="list-disc pl-5">
            {getRecommendedEvents(selectedEvent).map((event) => (
              <li key={`rec-${event.id}`} className="cursor-pointer text-blue-600 dark:text-blue-400 hover:underline"
        ...
        <div className="mt-6">
          <h3 className="font-semibold text-gray-800 dark:text-white mb-2">Комментарии:</h3>
          <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
            {(comments[selectedEvent.id] || []).map((comment, index) => (
              <div key={index} className="text-sm bg-gray-100 dark:bg-gray-700 p-2 rounded text-gray-800 dark:text-gray-200">
                {comment}
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 flex">
            <input
              ...
              className="flex-1 border border-gray-300 dark:border-gray-600 rounded-l px-3 py-1 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
      ...
      <p className="text-gray-700 dark:text-gray-300">Выберите событие на карте, чтобы увидеть информацию о нем.</p>
    )
  );
};
